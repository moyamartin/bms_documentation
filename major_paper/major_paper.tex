...
Teniendo esto en cuenta, analizamos lás matrices de evolución de nuestro modelo para hallar los polos  de mayor frecuencia, ya que estos determinan la dinámica de nuestro sistema.\\
En la figura \ref{system_bode} se observa que a partir de $\omega = 10 rad/s$ la ganancia esta determinada por $R_{0}$, por lo que tomamos este valor de referencia.\\
...
Calculando la frecuencia $F$ a partir de la frecuencia  angular $\omega$ obtenemos la máxima frecuencia que queremos muestrear y consecuentemente la mínima frecuencia de muestreo $f_{smin}$.

\begin{equation}
	F = \frac{\omega}{2 \pi} = 1.5915 Hz
	\label{ang_vs_frec}
\end{equation}

\begin{equation}
	f_{s min} > 2 F = 3.1830 Hz
