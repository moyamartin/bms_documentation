...
\subsubsection{Caracter\'isticas del pack}
...
Para poder desarrollar el algoritmo de ecualizaci\'on y estimaci\'on de
\acrshort{SOC}, es necesario obtener un modelo que permita representar de forma
matem\'atica el comportamiento de una celda NRC18650PF. Para tal fin se
desarrolla un modelo basado en un circuito el\'ectrico con 2 elementos RC en
serie, tambi\'en conocido como modelo de Randles de segundo orden, que se
encuentra representado por el esquem\'atico de la Figura \ref{randles_2rc} cuya
parametrizaci\'on se describe en la Secci\'on \ref{param_18650pf}.
...
\begin{figure}[h!]
    \begin{center}
        ...
        \draw  (8,0) to [open,v=$V_{out}v$,invert] (8,-3);
	    \end{circuitikz}
...
\acrshort{HPPC}.  Ese proceso involucra reiteradas simulaciones del modelo
matem\'atico planteada en conjunto de un algoritmo de optimizaci\'on num\'erico,
de esta forma, el proceso de optimizaci\'on busca ajustar los par\'ametros del
...
el comportamiento din\'amico de la bater\'ia, en m\'ultiplos niveles de
...
El set de datos de \cite{Kollmeyer2018} es un conjunto de ensayos realizados en 
...
En este caso, la curva HPPC de inter\'es se puede observar en la Figura
\ref{hppc_kollmeyer}, donde una ampliaci\'on sobre la respuesta din\'amica 
de la celda se puede observar en la Figura \ref{hppc_kollmeyer_zoom}
...
\subsubsection{Preprocesamiento del conjunto de datos}
...
\subsubsubsection{Segmentaci\'on de flancos}\label{flank_seg}
...
\subsection{Desarrollo del algoritmo de estimaci\'on de \acrshort{SOC}}
...
solo puede ser estimado, la estimaci\'on debe ser robusta con respecto a:
...
En este caso, el algoritmo de estimaci\'on del \acrshort{SOC} se basa en el uso 
...
observar para poder optimizar la ganancia de Kalman.
...
espacio de estados de la celda, como se muestra en la Ecuaci\'on 
\ref{ss_model_generic}:
...
Partiendo del modelo desarrollado en la Secci\'on \ref{dev_batt_model}
