...
\subsection{Técnica de Carga}\label{sec:tecnica_carga}
...
El circuito de entrada de nuestro cargador de batería consta de 2 partes, una
red de descarga RC y dos mosfets tipo P colocados en configuracion back to back
que aislan la fuente de alimentación del circiuto del cargador propiamente
dicho. 

\begin{figure}[h!]
    \begin{center}7
	\includegraphics[width=0.7\textwidth]{hardware/bat_charger/bc_input.png}
	\caption{Entrada de alimentación}
	\label{fig:cb_input}
    \end{center}
\end{figure}
\FloatBarrier
...
previene que fluya una corriente inversa de descarga desde la batería en caso de
que se encuentren apagados y minimizan la disipación de energía a partir de la
baja resistencia $R_{DS(on)}$ que presentan operando en directa, en comparación
a un diodo Schottky. Q8 a su vez provee al circuito la capacidad de variar la
corriente de alimentación de forma controlada $di/dt$ al conectar la fuente de
alimentación al circuito mediante el control del tiempo de encendido.
    
Durante una conexión en caliente, la presencia de picos de sobre voltaje en la
entrada pueden afectar al circuito integrado. Esto se debe al sistema de segundo
orden formado por las inductacias y capacidades parácitas que pueden presentarse en
el cable de la fuente de alimentación. R38 y C24 en serie fornan un circuito
de descarga que elimina los picos de sobre tensión producidos por estos
transitorios.

El diseño del circuito RC snubber se realizo teniendo en cuenta que la
disipasión de energía en el mismo no es crítíca. Calculamos R38 bajo la
condición de que permita fluir una corriente suficiente que limite un posible
sobre voltaje.  La disipasión de energía en R38 es independiente de su valor ya
que disipa la energía almacenada en el capacitor en serie con la misma.

%TODO: add Rc snubber calcs

\subsubsubsection*{Control de Corriente Constante}

[Texto generico de perfil de corriente]

\begin{figure}[h!]
    \begin{center}
	\includegraphics[width=0.4\textwidth]{hardware/bat_charger/bc_set_points_I.png}
	\caption{Circiuto de fijación de Set-Point de I}
...
entre $V_{ref} \equiv 3.3$ y masa como se puede observar en la figura
...
$I_{max}$ en 5A seleccionamos el valor de la resistencia shunt sensora de la
corriente de salida $R_{SR}$. La diferencia de potencial máxima admitida entre
los pines SRP y SRN $\Delta V_{SR Max}$ es 100mV, así que calculamos el mayor
valor de resistencia posible que nos permita maximizar la resolución del sensado
de corriente. 

\begin{equation}
    \begin{split}
	&R_{SR}=\frac{I_{max}}{\deltaV_{SR max}}\\
	&R_{SR}=\frac{5A}{100mV}\\
	&R_{SR}=20m\ohm
    \end{split}
\end{equation}
...
carga máxima de la misma debe ser 0.5 veces la capacidad nominal la misma.
%TODO: Citar hoja de datos
Por lo tanto, debido a la configuración 3P6S de nuestro pack de batería con 3
celdas en paralelo la corriente de carga rápida del mismo deberá ser fijada en $
...
\begin{equation}
	...
	&V_{ISET1}=4125mA*20*20m\ohm\\
	&V_{ISET1}=1.65v
    \end{split}
\end{equation}

y a partir de conocer la tensión a aplicar en ISET1 es que dspejamos el divisor
...
\begin{equation}
	...
	&V_{ISET2}=300mA*100*20m\ohm\\
	&V_{ISET2}=600mV
    \end{split}
\end{equation}

y a partir de conocer la tensión a aplicar en ISET2 es que dspejamos el divisor
...
[Set-Point sensado corriente de carga]

\subsubsubsection*{Control de Tensión Constante}
...
\begin{figure}[h!]
    \begin{center}
	\includegraphics[width=0.3\textwidth]{hardware/bat_charger/bc_set_points_v.png}
...
El voltaje aplicado en VFB es regulado a 2.1V al comparar el mismo internamente
contra una referencia de 2.05V la cual define si el voltaje de salida se haya
por debajo o por encima del valor deseado. 
...
de 0.5\%.
Vale aclarar que el divisor resistivo queda en paralelo con la batería
y nos cersioramos que el drenado de corriente no descargue el pack de baterría
de forma incipiente. La corriente drenada por el divisor es de $42\mu
A=\frac{2900mAh}{42\mu A}*3=207142hs\equiv 8630 días$ tiempo que tardará en
descargar completamente el pack de batería.

\subsubsubsection*{Salida}

El circuito de la figura \ref{fig:cb_output} corresponde a la etapa de salida del
cargador tipo buck implementado. 

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{hardware/bat_charger/bc_output.png}
    \caption{Circuito de salida}
    \label{fig:cb_output}
\end{figure}
\FloatBarrier

El bq24610 tiene una frecuencia de switcheo de salida de 600kHz lo que nos
permite seleccionar y utilizar inductores y capacitores de salida d menor valor.

Según la hoja de datos del bq24610, para una corriente de salida de 4A la
inductancia típica recomendada es $6.8\mu H$.
La selección de la inductancia
de salida la realizamos siguiendo los lineamientos y recomendaciones de la hoja
de datos del \acrshort{CI} para luego calcular y corroborar los valores
seleccionados. 

Los parámetros de referencia tomados para los cálculo de los componentes del
circuito de salida son la tensión de alimentación máxima $V_{In max} = 30V$, la
tensión de salida máxima $V_{Out max} = 25,2V$, la corriente de salida máxima
$I_{Out max}=5A$ y la frecuencia de switcheo del circuito tipo buck $f_{s}=600
KHz$ 
...
\begin{equation}
    I_{ripple}=\frac{V_{In max}*D*(1-D)}{f_{s}*L}=\frac{30*0.620*(1-0.620)}{600
    kHz*6.80\mu H}=1,73A
\end{equation}

\begin{equation}
    6.5A=I_{Sat}\geq I_{chg} +\frac{1}{2}I_{ripple}=5.865
\end{equation}

\begin{equation}
    L=\frac{(V_{In max}-V_{Out max})*V_{Out max}}{V_{In max}*f_{s}*R*I_{Out
max}}=\frac{30V-25.2V)*25.2V}{30V*600kHz*0.3\ohm*5A}=4.48\mu H
\end{equation}

El capacitor de salida fue seleccionado teniendo en cuenta que tenga una
cantidad de corriente de ripple suficiente para absorver el ripple de la
corriente del switching de salida.

\begin{equation}
    I_{C out}=\frac{I_{ripple}}{2*\sqrt{3}}\approx 500.7mA
\end{equation}

El ripple de voltaje del capacitor de salida lo calculamos como: 
\begin{equation} 
    \Delta V_{O}=\frac{1}{8*L*C*f_{s}²}(V_{Out max}-\frac{V_{Out
    max}²}{V_{In max}})=6.43mV
\end{equation}
...
información necesaria del estado del proceso de carga, habilitarlo y
desahilitarlo.
...
partir del switch SW2 que permite independizar el cargador de la unidad de
