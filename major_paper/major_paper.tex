...
\begin{abstract}
    ...
    transporte con carga y busca resolver muchas de las problem\'aticas
    ...
    \noindent A pesar de estar diseñado para veh\'iculos el\'ectricos, el
    mismo puede ser implementado con almacenadores de energ\'ia, tales como los
...
generan el deterioro del medio ambiente, entre ellas, se encuentra el 
...
f\'acilmente en un proyecto relacionado.
...
El informe se encuentra dividido en 9 Cap\'itulos que atienden a los siguientes 
enfoques:

\begin{itemize}
	\item Cap\'itulo \ref{descripcion}: Se realiza una descripci\'on en alto 
        nivel del proyecto, detallando los objetivos generales, las 
        especificaciones y requerimientos del trabajo.
	\item Cap\'itulo \ref{teoria}: Se encuentra el fundamento te\'orico, donde 
        se detalla el funcionamiento de una celda de Litio-Ion, los fundamentos 
        b\'asicos de modelados de celdas y algoritmos de estimaci\'on de carga, 
        como tambi\'en aquellos relacionados a la equalizaci\'on de celdas. 
        Adem\'as, se presenta el desarrollo matem\'atico de muchos de los 
        m\'etodos junto con el planteo de las ventajas y desventajas que ofrecen 
        uno sobre el otro.
	\item Cap\'itulo \ref{preambulo}: Describe brevemente el desarrollo e
        implementaci\'on del proyecto, otorgando al lector informaci\'on previa
        de las especificaciones t\'ecnicas del proyecto sobre como fue
        desarrollado el modelo, el hardware y el software del mismo.
	\item Cap\'itulo \ref{ch:dev_soc}: Se describe el desarrollo del modelo 
        mátemático de la celda de Ion-Litio y su implementación en un Filtro de 
        Kalman.
	\item Cap\'itulo \ref{desarrollo_hw}: Se describe el diseño e implementación 
        del soporte físico del BMS, esto incluye la selección de los circuitos 
        integrados y el dimensionamiento de los componentes.
	\item Cap\'itulo \ref{software_implementation}: Se describe la estructura de 
        software desarrollada para la implementaci\'on del modelo de la celda en
        el dispositivo f\'isico, como tambi\'en los drivers de los 
        perif\'ericos utilizados y, finalmente, como se integran los distintos
        m\'odulos en una m\'aquina de estados.
	\item Cap\'itulo \ref{ensayos}: Se demuestran los ensayos realizados a lo 
        largo del proyecto para validar los modelos y el funcionamiento del
        hardware.
	\item Cap\'itulo \ref{conclusiones}: Se concluyen sobre la experiencia del 
        proyecto demostrando los conocimientos adquiridos, presentando futuras 
        mejoras y extensiones del mismo.
\end{itemize}
...
\chapter{Desarrollo e implementación - Preámbulo}\label{preambulo}
\thispagestyle{fancy}

A partir de este Cap\'itulo se presenta el desarrollo matem\'atico y la
implementaci\'on f\'isica del \acrshort{BMS}, incluyendo el pack de bater\'ias,
el modelo matem\'atico de la celda de Litio-Ion junto a sus algoritmos de
estimaci\'on de \acrshort{SOC} y ecualizaci\'on de celdas, mostrando los
resultados de las simulaciones y, finalmente, el desarrollo del hardware y
firmware necesario para implementar los distintos componentes del sistema en un
sistema f\'isico real.  

\section{Espíritu del proyecto}

El desarrollo del proyecto se funda sobre bases de espíritu colaborativo, bajo
...
\chapter{Desarrollo e implementación de un Filtro de Kalman para la estimación
del \acrshort{SOC} de celdas de Li-Ion}\label{ch:dev_soc}
\thispagestyle{fancy}

Como hemos presentado a través de la Sección \ref{algSoc}, existen
diferentes opciones para el cálculo del \acrshort{SOC} del
pack de baterías de un \acrshort{BMS}. Durante el siguiente capítulo se
presenta el proceso de desarrolo e implementación de un algoritmo de cálculo del
\acrshort{SOC} de un pack de baterías de Litio-Ion a partir de la implementación
de un Filtro de Kalman Extendido como se describe en la Sección
\ref{KalmanFilterMethod}.

El \'exito de la implementación del algoritmo de estimación de \acrshort{SOC}
seleccionado depende a su vez de contar con un modelo preciso de la celda de
Litio-Ion que sea capaz de representar los datos experimentales de ensayo de
laboratorio de la misma. El proceso de selección de la celda, de la obtención de
los datos experimentales, su procesamiento y, de la selección y desarrollo de un
modelo de la celda de Litio-Ion son tambi\'en presentados a continuación. 

\section{Celda Panasonic NCR18650PF} \label{seq:cel_selection}

El modelo da la celda de Litio-Ion 18650 fue originalmente diseñada para ser
utilizada en bater\'ias de notebooks pero debido a su amplia versatilidad su uso
fue expandiéndose con el tiempo a un sin número de dispositivos entre ellos los
relacionados a las implementaciones en \acrshort{VVEE}s.

Estas bater\'ias se caracterizan por su tamaño reducido de 18mm de di\'ametro
por 65mm de alto, características que le dan su nombre. En el mercado actual, se
pueden encontrar diferentes versiones del mismo modelo debido a que cada uno
posee una composici\'on qu\'imica diferente, dando lugar a las distintas
caracter\'isticas de capacidad y voltaje que se describen a continuaci\'on:
...
implementaciones de packs de bater\'ias de uso comercial para vehículos eléctricos
y en la disponibilidad en el mercado que esto conlleva. A su vez ha sido un
factor determinante para su selección la existencia y disponibilidad de un set
de datos completos \cite{Kollmeyer2018}, abiertos y libres de distintos ensayos
sobre la celda en cuestión a partir del cual nos ha sido posible desarrollar e
investigar el modelo de la celda, correr simulaciones de la misma y plantear
diferentes algoritmos de medición del \acrshort{SOC}. Datos a partir de los
cuales fundamentalmente se validaron los resultados obtenidos.  

La celda NCR18650PF de Panasonic se caracteriza por la composición química de su
cátodo, basada en el óxido de litio níquel cobalto aluminio
($\mathrm{LiNiCoAlO_2}$) y, según su hoja de datos \cite{18650pf}, sus
carater\'isticas el\'ectricas se pueden observar en la Tabla 
\ref{ncr18650pf_table}.
...
El set de datos experimentales utilizado es un conjunto de ensayos realizados en
la Universidad de Wisconsin-Madison por el Dr. Phillip Kollmeyer
\cite{Kollmeyer2018}.

Estos ensayos fueron realizados sobre una celda Panasonic NCR18650PF de 2.9Ah en
una c\'amara t\'ermica con una celda de carga marca \emph{Digatron} que cuenta
con una capacidad de descarga de 25A a 18V.
...
El trabajo se encuentra centrado puntualmente en la descripción del ensayo 
\acrshort{HPPC} ya que es a partir del cual se calculan los parámetros internos 
del modelo de la celda de Litio-Ion que se presenta en la Sección 
\ref{dev_batt_model}.

\subsubsection{Ensayos HPPC}\label{ensayo_HPPC}

El ensayo \acrshort{HPPC} es una metodología utilizada para poder caracterizar
...
diferentes intervalos del estado de carga (\acrshort{SOC}). Para alcanzar los
...
estado de carga del ciclo de ensayo siguiente.
...
se presentan \'unicamente pulsos múltiples de descarga y un periodo de 
relajación entre intervalos del 5\% del \acrshort{SOC}. El pulso de descarga 
entre intervalos del \acrshort{SOC} no está presente en el set de datos.
...
ensayo perturbe la celda sin modificar su estado de carga.
...
de estos pulsos nos brinda, no solo información respecto de la tensión de
circuito abierto (\acrshort{OCV}), sino también de la evolución de la celda
frente a una excitación por corriente a determinados \acrshort{SOC}s.
...
comportamiento y la dinámica de una celda de Ion-Litio es condición \emph{sine
qua non} para la implementación exitosa de los algoritmos de estimación del
...
En la presente Sección se describe el trabajo realizado para definir las
...
tratamiento del set de datos experimentales utilizado, presentado en la Sección
\ref{ensayo_HPPC}, como por el proceso intrínseco de cálculo y posterior trabajo
recursivo de optimización para ajustar los parámetros y con ellos el 
comportamiento del modelo a los datos experimentales validando los mismos.
...
El modelo puede ser re-escrito en el dominio de la transformada de Laplace como: 
...
com\'unmente usado para ajustar la respuesta de un modelo matemático a los datos
...
forma, el proceso de optimizaci\'on busca ajustar los par\'ametros internos del
modelo para minimizar el error entre los datos experimentales y los resultados
de las simulaciones realizadas. En el caso de las celdas de litio-ion, los datos
experimentales fueron obtenidos a partir de ensayos \acrshort{HPPC} cuyo perfil
fue descripto en la Sección \ref{ensayo_HPPC}. 
...
celda. Para realizar esto, se emplearon tablas de b\'usqueda (\acrshort{LUT}, 
del ingl\'es \emph{\acrlong{LUT}}).

Entonce se puede concluir que el modelo, para poder representar con exactitud la 
dinámica de una celda en cada uno de los escalones de \acrshort{SOC}, requiere 
de múltipes juegos de parámetros internos. Un juego por cada escalón de descarga 
de corriente que presente el ensayo \acrshort{HPPC} y a su vez, otro juego 
completo de parámetros por cada temperatura de ensayo distinta en caso de querer 
ajustar el modelo a las variaciones de temperaturas.
...
en la Figura \ref{hppc_kollmeyer}, las que nos brindan la información necesaria.

\subsection{Validación del número de ramas R-C del modelo}

Una vez estudiados los ensayos \acrshort{HPPC} surge el interrogante de sí el
modelo seleccionado en la Sección \ref{dev_batt_model} para representar la 
celda de Litio-Ion será capaz de describir la dinámica que presentan las celdas
ensayadas. 

Para validar el modelo se propone que el mismo sea capaz de describir la 
dinámica completa de una celda de Ion-Litio ensayada siguiendo el procedimiento 
enunciado en \cite{Jackey2013BatteryMP} para la selección del número de ramas 
R-C, que consiste en presentar un modelo y ajustar 2 ecuaciones exponenciales, 
una de orden 1 (Ec. \ref{eq:exp1}) y otra de orden 2 (Ec. \ref{eq:exp2}), a la 
curva que traza los datos del ensayo HPPC utilizando la herramienta \emph{Curve
Fitting} de Matlab.
...
Se selecciona espec\'ificamente la fase de relajación de uno de los escalones
...
la din\'amica fundamental es descripta por el número de ramas R-C en serie
dispuestas en el circuito equivalente. 

\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=.65\textwidth]{rc_model_fit.png}
        \caption{Ajuste de las curvas exponenciales al modelo R-C}
        \label{fig:model_fit}
    \end{center}
\end{figure}
\FloatBarrier
...
que dos t\'erminos exponenciales temporales, el equivalente a dos ramas RC 
paralelas, son suficientes para describir la dinamica de una celda de
...
mantener el costo computacional bajo a los fines de embeber el modelo en un
microcontrolador. 
...
El modelo nos permite observar la evoluci\'on de la tensión en bornes de la 
celda ante las variaciones de sus variables de entrada a partir de la medición 
de la tensi\'on de salida que simula el mismo.

El modelado de los componentes a partir de una \acrshort{LUT} no contempla
...
temperatura como en corriente de excitación, requerir\'an simplemente repetir el
...
seleccionados para $\mathrm{C_{1}, C_{2}, R_{1}, R_{2}}$ responden solamente a 
ensayos experimentales previos y tienen unicamente el objetivo de ejercitar el 
\emph{simulink} para corroborar su correcto funcionamiento.
En el caso de $R_{0}$ su valor fue obtenido promediando todos los valores de
$R_{0}$ calculados para cada uno de los pulso de corriente del ensayo HPPC.

\begin{itemize}
    \item $C_{1} = 172.28 F$
    \item $C_{2} = 1485.47 F$
    \item $R_{1} = 0.0273 \mathrm{\Omega}$
    \item $R_{2} = 0.1200 \mathrm{\Omega}$
    \item $\mathrm{R_0}=0.0256\mathrm{\Omega}$
\end{itemize}

\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=1\textwidth]{v_sim_v_dataset_no_opt.png}
...
las simulaciones o la segmentaci\'on o modificaci\'on de los datos \'utiles para
...
Para poder rescatar el valor del \acrshort{OCV} a partir del ensayo 
\acrshort{HPPC} y construir la curva que se presenta en la
Figura \ref{fig:ocv_detection} es necesario seleccionar criteriosamente el punto
 en donde tomar el valor de voltaje en bornes de la celda para asegurar que la 
 celda ensayada haya alcanzado el mayor estado posible de relajación antes de 
 tomar el valor de \acrshort{OCV} para asegurar que su tensión en bornes 
 equivalga al mismo.

Durante todo el ensayo \acrshort{HPPC} se observan per\'iodos de descanso
lo suficientemente largos como para considerar que la bater\'ia ha alcanzado la
...
en los puntos donde un flanco negativo fue detectado, como se describe en la
Secci\'on \ref{flank_seg} y, posteriormente, este valor es extrapolado hasta el 
flanco negativo inmediatamente anterior como se observa en la figura
...
Esta curva se puede observar en la Figura \ref{fig:ocv_vs_soc}, que representa
el funcionamiento de la fuente de voltaje controlada por \acrshort{SOC} en el
esquem\'atico de Randles.

\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.9\textwidth]{ocv_vs_soc.jpg}
        \caption{Curva \acrshort{OCV} Vs \acrshort{SOC}}
...
prueba y error hasta obtener un proceso funcional acorde a los resultados
...
Una vez identificados todos los flancos se segmenta el set de datos en porciones
abarcables por el algoritmo de optimización. En la Figura 
...
circulando por el modelo y una zona de relajación a partir de la que el pulso de
...
instant\'aneo de voltaje que representa $R_{0}$. La zona de excitación no 
...
partir de la implementación en MATLAB de un algoritmo recursivo de optimización,
...
condiciones iniciales y de limites de los mismos. El c\'odigo a su vez prepara 
el modelo para ser ejecutado en cada tarea de estimación y ajuste seleccionando 
los intervalos correctos de las señales de entrada del modelo y sus condiciones
iniciales.

Los límites de los parámetros a estimar se setean de forma experimental tomando
en cuentas los resultados de cálculos matem\'aticos realizados previamente y de
...
y bloques para el tratamiento de modelos. El \emph{toolbox} provee la capacidad 
de interacción entre el modelo de la celda desarrollados con la librer\'ia
...
El método matem\'atico de optimización seleccionado es el de mínimos cuadrados no
...
curva de mínimos cuadrados no lineales de la forma descripta en la Ecuaci\'on
\ref{min_cuad_form}

\begin{equation}
    \min_{x}\|f(x)\| = \min_{x} (f_1(x)^2 +
    f_2(x)^2+\dots+f_n(x)^2)\label{min_cuad_form}
\end{equation}
...
computacionalmente más rápido en comparación a otros metodos para la solución 
...
comparación gráfica de los datos obtenidos de la simulación respecto de los 
datos experimentales antes y despu\'es de ejecutado el proceso de optimización, 
...
cada nivel de \acrshort{SOC} la simulación del 5to escalón de corriente no
logra ajustarse a los datos experimentales como se observa, a modo de ejemplo,
en la Figura \ref{fig:exp_vs_sim_5_error}.

Este fenómeno se explica debido a que el dataset no incluye la información de
los periodos de descarga largos que llevan la celda al siguiente escal\'on de
\acrshort{SOC} de ensayo. Por lo cual, estos pulsos carecen de la información
suficiente respecto del estado de relajación final de la celda una vez
finalizado el pulso de corriente. Por otra parte, no es casualidad que el
fenómeno se presente en los pulsos de descarga de mayor intensidad, siendo los
...
Viendo que el patrón se repite sistemáticamente en los pulsos 5tos de cada uno
...
así que se concluye el proceso de cálculo y optimización con 55 juegos de
par\'ametros correspondientes a 55 \acrshort{SOC} de la celda que modelan la
...
optimizadas para el circuito equivalente.
...
Se validan los juegos de parámetros finales corregidos sometiendo el modelo
...
\ref{fig:DC_modVsData} se puede apreciar que la simulación del drive cicle
...
\acrshort{BMS} apropiado. Como el mismo solo puede ser estimado, esta
estimaci\'on debe ser robusta con respecto a:
...
presente trabajo se desarrollan dos algoritmos, uno basado en un filtro
observador, como lo es el Filtro de Kalman, cuyo desarrollo se describe a 
continuaci\'on y otro algoritmo basado en redes neuronales.
...
a su vez es desarollada matricialmente, como se puede observar en la
...
obteniendo el Sistema de Ecuaciones de Estados \ref{randles_ss_incomplete}.
...
se analiza el circuito RC paralelo de la Figura \ref{tanque_rc}
...
sistema de ecuaciones de estados es observable, si, y s\'olo si, su matriz de
observabilidad, valga la redundancia, es observable, es decir, si su 
determinante es diferente a cero y la matriz de observabilidad tiene un rango n 
...
\ref{obs_matrix_randles} obtenemos la Ecuaci\'on \ref{det_obs_matrix_randles}
...
Por lo tanto la matriz O es observable, si, y s\'olo si, se cumple que los
...
Por \'ultimo, considerando un sistema de ecuaciones de la siguiente forma 
\emph{(Ecs. \ref{ss_with_noise})},
...
Para poder implementar el Sistema de Ecuaciones en la Ecuaci\'on 
\ref{ss_kalman_gain} sobre el modelo de Randles es necesario proveer las 
matrices que proporcionan valores de varianza sobre las incertezas del modelo 
(Q) como tamb\'ien los valores de varianza sobre el ruido al que est\'an 
sometidas las variables de medici\'on (R), por el otro lado, tambi\'en es 
necesario proveer el estado inicial del sistema ($x_0$) como tambi\'en su 
covarianza inicial ($P_0$).
...
analizaron los resultados obtenidos del modelo en la Secci\'on 
\ref{val_param_modelo}, teniendo en cuenta las variaciones de las tensiones de 
cada capacitor en conjunto con los valores de \acrshort{OCV}(SOC), se acotaron y
...
utilizado en \ref{ensayo_HPPC}. Como resultado se obtuvieron las gr\'aficas que 
se observan en la Figura \ref{kalman_result_matlab}, en ellas se puede deducir 
que se obtiene un error m\'aximo de 3\%, aceptable para los objetivos del 
trabajo.

\begin{figure}[h!]
    \begin{center}
        \includegraphics[width=0.8\linewidth]{kalman_result_matlab.png}
...
\begin{figure}[h!]
    \begin{center}
        ...
        % draw rectangle around the battery pack
            \draw [dashed] (-.6, 2.6) rectangle (2.6, -2.6);

        % each of these are 6 cells in series %
            \draw (0, 2) -- (0, 2.2);
        ...
        % cell balancing and voltage sensing block %
        ...
        % connect 6 series cells to balancing module %
        ...
        % Conexion entre el MCU y el balanceador % 
...
32-bits Arm® Cortex®-M4 con arquitectura \acrshort{RISC} (del ingl\'es,
\acrlong{RISC}) y que puede operar a una frecuencia de hasta 168 MHz. 
El n\'ucleo Cortex-M4 implementa dos módulos fundamentales para la optimización 
del proceso de cálculo matemático del \acrshort{BMS} y el procesamiento en 
tiempo real. Una unidad de punto flotante (por sus siglas en ingles 
\acrfull{FPU}) de precisi\'on simple que soporta todas las instrucciones y tipos 
de datos Single-Precision data-Processing del n\'ucleo y un set completo de 
instrucciones de \acrshort{DSP} (del ingl\'es, \acrlong{DSP}). La familia 
STM32F407xx a su vez incorpora una memoria FLASH embebida de alta velocidad de 
hasta 1 Mbyte y 192+4 Kbytes de RAM (\acrfull{SRAM}).
...
Para incrementar su rendimiento durante los transitorios de l\'inea y carga y para
...
Este dispositivo se encarga de mantener las celdas ecualizadas, para llevar a 
cabo esta tarea se implementa sensores de tensión para cada celda individual, 
además de reportar toda la información a la unidad de procesamiento.
Por otro lado, también se encarga de accionar los transistores de ByPass según 
lo determine el algoritmo de balanceo de las celdas en base al estado de carga 
estimado por el Filtro de Kalman.

Dada las caracter\'isticas del pack descriptas en la Secci\'on 
\ref{Section_battery_pack} y la diversidad de t\'ecnicas disponibles para el 
balanceo, con el objetivo de mantener un diseño simplificado agregando la 
funcionalidad de ecualizaci\'on al producto final, se opt\'o por un balanceador 
basado en una topolog\'ia pasiva utilizando resistencias shunt en conjunto con 
una estrategia de control cl\'asica que depende del promedio del 
\acrshort{SOC} entre las 6 celdas, estableciendo un umbral m\'aximo de 
variaci\'on entre celdas como criterio de balanceo. 
...
En la Figura \ref{fig:bq76_implementation_uncouple} se muestran todos los 
capacitores de desacople especificados por el fabricante \cite{BQ76PL536A}:
...
\textbf{2N7002} de la firma \emph{ON Semiconductors} en su empaquetado 
$SOT\textendash23$ junto con las resistencias R15, R18 y el diodo D1 que forman 
parte del circuito de encendido del MOSFET. Y de una resistencia, R21 en el caso 
del ejemplo presentado en la Figura \ref{fig:bq76_balance_power}, que hace las 
veces de resistencia \emph{shunt}, según se describe en la Sección
\ref{seq:balanceo_pasivo}, encargada de disipar la energía excesiva acumulada en
la celda desbalanceada vali\'endose del voltaje de la misma. 

La estrategia de ecualización es implementada y procesada en la unidad de
...
ecualizaci\'on $I_{ecu} \equiv 29mA * 3 = 87 mA$

Cada una de las entradas de ADC de medición de tensión de las celdas requieren
la presencia de un filtro anti-aliasing. El propósito del filtro es eliminar
toda señal indeseada con frecuencias superiores a frecuencia muestreo. 
Cada filtro consiste en una resistencia de $1 K\ohm$ en serie con un capacitor 
de $100 nF$ en los pines VC6 - VC2 y de $1 \mu F$ en VC1 y VC0.

La sección de potencia del balanceador cuenta con otro circuito de proteccion de
sus integrados. Un circuito \emph{dumper} para evitar transitorios de sobre 
voltaje por la conexión del pack de batería en caliente. 

\section{Sensor de corriente}

Este dispositivo es el que provee el valor de corriente que circula a través 
de las celdas hacia la carga o desde el cargador. El dato de la corriente es 
clave para el correcto funcionamiento del Filtro de Kalman y nos permite tomar 
acciones de seguridad en caso de valores de corriente elevados o variaciones
bruscas de la misma.
...
monitorea simult\'aneamente la tensión que cae en una resistencia shunt y la del 
...
\begin{figure}[h!]
	...
	\caption{Implementación t\'ipica - INA226}
...
\begin{figure}[h!]
	\begin{center}
		...
		\end{minipage}
	\end{center}
	\caption{Modelos de transistor mosfet en zonas de saturación y \'ohmica 
             respectivamente.}
	\label{modelo_mosfet}
\end{figure}
\FloatBarrier
...
caracter inductivo, surge la necesidad de realizar el siguiente análisis sobre 
...
cortocircuitos a la alimentación, dado que el chasis \'integro del veh\'iculo
est\'a conectado a masa. Para un interruptor \emph{low-side} un cortocircuito a 
masa provoca el encendido de la carga. Para el caso del \emph{high-side} un
cortocircuito en su salida generará una sobrecorriente en el interuptor por lo
que se deben tener en cuenta determinadas protecciones para el interruptor.
...
valor de $R_{on}=4.6 m\ohm$ minimizando p\'erdidas en conducción. El circuito de
...
Implementa un convertidor de energía DC-DC del tipo buck sin aislamiento que
...
Emplea un controlador de corriente y de tensión sincrónico \emph{switching} 
\acrshort{PWM} de frecuencia constante y de alta resolución cuya topología 
simplificada puede observarse en la Figura \ref{fig:simp_sch_char}. 
...
Admite la conexión de hasta 6 celdas en serie con una tensión y corriente máxima 
de entrada de hasta $28V$ y $10A$ respectivamente y una salída de hasta $26V$ y 
$10A$ ajustandose adecuadamente a nuestros requerimientos y a la topología de 
nuestro pack de baterías \emph{6S3P} descripta en la Secci\'on 
...
\begin{tabular}{ll}
    ...
    $V_{DS (on)}=37.55mV$ &Ca\'ida de voltaje en elemento switching Q9\\
    $f_{s}=600 KHz$ &Frecuencia switching\\
\end{tabular}

\subsubsection{Alimentación}

El circuito de alimentación del cargador de batería consta de una red de 
descarga RC, de dos MOSFETs canal-P colocados en configuración back to back con 
el objeto de aislar la fuente de alimentación externa del circuito del cargador 
propiamente dicho y de una resistencia shunt de sensado de la corriente de 
entrada. 
...
las inductancias y capacidades parásitas que pueden presentarse en el cable de 
la fuente de alimentación. R38 y C24 en serie forman un circuito de descarga que
...
corte en los MOSFETs de entrada por parte del \acrshort{CI} siguiendo los
...
La disipación de energía en $R_{S}$ es independiente de su valor y de la
...
$I_{Out (max)}=5A$ se selecciona el valor de la resistencia shunt que sensa la
...
Si el pack de baterías se encontrase profundamente descargado, el bq24610
...
La corriente de precarga $I_{pre charge}$ y de fín de carga $I_{cuttoff}$ se 
desarrolla entonces en la Ecuaci\'on \ref{eq:ICutOff},
...
La ecuación que define el voltaje de carga $V_{Charge} \equiv V_{VFB}$ será 
entonces:
...
A partir de conocer la tensión de carga $V_{Charge}$ se despeja el divisor
...
batería. Es por esto que se cerciora que el drenado de corriente por el
...
por R53 y R54. La corriente de entrada es sensada en la resistencia shunt R39,
...
que funciona como bootstrap del voltaje necesario para excitar el gate del 
mosfet de la parte alta. Ambos componentes pueden observarse en la Figura 
\ref{fig:bc_ic}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{hardware/bat_charger/bc_output.png}
    \caption{Etapa de potencia - Cargador de Bater\'ia}
    \label{fig:cb_output}
\end{figure}
\FloatBarrier
...
condiciones de operación en la Ecuación \ref{eq:cb_L} seg\'un lo enunciado en 
las notas de aplicación \cite{ROHM_Switching_AN}.
...
4A, la inductancia seleccionada sea de $6.8\mu H$. Se adopta este valor, mayor 
al calculado, para disminuir la relación entre la corriente de ripple y la 
corriente de salida.
...
circulación de corriente suficiente a los fines de la implementación.
...
Otras magnitudes tenidas en cuenta para la selecci\'on de capacitor de salida
fueron la corriente de ripple máxima que deber\'a soportar el integrado.
...
menores son las perdidas totales. 
...
No son presentados en el informe los cálculos de los diferentes MOSFETs que se
...
función de ciclo de trabajo D, la corriente máxima de salida $I_{out (max)}$, 
la resistencia de encendido del mosfet $R_{DS(on)}$, el voltaje de alimentación
...
esta funcionalidad y recomendamos que la carga del pack de batería sea
...
El pack de bater\'ias fue ensamblado utilizando soportes encastrables como se
observa en la Figura \ref{soporte_18650}, impresos utilizando una impresora 3D, 
donde cada celda es conectada entre s\'i utilizando una banda de níquel y la
...
El pack ensamblado se puede observar en las Figuras 
\ref{battery_top}-\ref{battery_side}
...
Para la implementación del protocolo CAN en dispositivos se utilizan 
generalmente dispositivos denominados transreceptores, los mismos son los 
encargados de traducir los niveles de tensión diferencial del BUS a niveles 
referidos a tierra aptos para el microcontrolador en nuestro caso 0 a 3.3V. 
...
Ultra-bajo consumo siendo \'optimo para aplicaciones portables como un vehículo 
eléctrico.
...
Las resistencias R6 y R7 en conjunto a C61 conforman una terminación de bus 
...
\acrshort{MCU} y programado como puerto serie (o \acrshort{UART}, del ingl\'es
\acrlong{UART}), la misma se puede observar en la Figura \ref{UART_Connector}, 
\'esta no se encuentra preparada para la utilización del usuario ya que no posee 
circuitos de protección ni aislación, la incorrecta utilización del mismo puede 
provocar serios daños al \acrshort{BMS}.
...
Figura \ref{placa_bms_fotografia} se puede observar una foto del circuito 
impreso ensamblado. 
...
\acrlong{HAL}), que provee una serie de \acrshort{API}s para que las capas 
superiores (aplicaciones, librerias y el stack) puedan interactuar con los 
recursos del hardware, en un lenguaje \emph{entendible} para el desarrollador.
...
\emph{src}) y sus definiciones dentro librer\'ias (carpeta \emph{inc}) para que 
cada m\'odulo pueda ser reutilizado sin problemas.
...
\section{Desarrollo de Módelo Máquina de Estado Finitos}

Para llevar a cabo las tareas previamente mencionadas, se opt\'o por un modelo 
de máquina de estados debido a su capacidad de representar sistemas 
arbitrariamente complejos en un conjunto de estados y transiciones, lo cual 
simplifica la implementación, reduciendo la consigna a un grupo de tareas 
pequeñas y condiciones simples.
...
\begin{itemize}
	...
	\item Flexible - Soporta instanciar objetos de tipo máquina de estado
    \item Escalable - Permite implementar transiciones y eventos de forma
        gen\'erica permitiendo expander f\'acilmente una m\'aquina de estados en
...
externos pueden ser generados por cualquier función externa que puede provenir
de otra máquina de estados y dispositivo externo (por ejemplo, una
interrupci\'on), lo que nos permite relacionar los diferentes subsistemas del 
BMS.

El sistema se encuentra dividido en 4 grandes funcionalidades

\begin{itemize}
    \item Sistema General de Monitoreo de Baterias - Figura \ref{fig::BMS_FSM}
    \item Cargador de baterias - Figura \ref{fig::Charger_FSM}
    \item Estimación de \acrshort{SOC} - Figura \ref{fig::SOC_FSM}
    \item Ecualización o balanceo en base al\acrshort{SOC} - Figura
...
\begin{figure}[h!]
	\begin{center}
        ...
        \caption{\acrshort{FSM} Principal del BMS} 
		\label{fig::BMS_FSM}
	\end{center}
\end{figure}
\FloatBarrier
\begin{figure}[h!]
	\begin{center}
        ...
        \caption{\acrshort{FSM} de estimación de \acrshort{SOC}} 
		\label{fig::SOC_FSM}
	\end{center}
\end{figure}
\FloatBarrier
\begin{figure}[h!]
	\begin{center}
        ...
        \caption{\acrshort{FSM} del cargador de batería} 
...
\begin{figure}[h!]
	\begin{center}
        ...
        \caption{\acrshort{FSM} del balanceador} 
		\label{fig::Equalizing_FSM}
	\end{center}
\end{figure}
\FloatBarrier

\section{Drivers propietarios}
...
Las mismas son descriptas a continuación de forma detallada.

\subsection{Driver \texttt{bq76}}\label{bq76_driver}
...
\begin{itemize}
    \item \texttt{bq76\_init()}: Inicializa una instancia del driver bq76 y
        el \acrshort{CI} con los parámetros de configuración seleccionados.
    \item \texttt{bq76\_broadcast\_reset()}: Envia un comando \emph{reset} a 
        todos los dispositivos instanciados con una dirección válida de 
        comunicación.
    \item \texttt{bq76\_reset()}: Reinicia un dispositivo espec\'ifico.
    ...
    \item \texttt{bq76\_set\_cuvt\_config()}: Aplica lo mismo que la
        funci\'on \texttt{bq76\_set\_covt\_config()}, pero para el caso de baja
        tensi\'on de una celda.
    \item \texttt{bq76\_set\_ot\_config()}: Establece el valor m\'aximo de
        ...
        implementa igual para su futuro uso.
    \item \texttt{bq76\_set\_ott\_config()} Establece el tiempo que tarda el
        el dispositivo en notificar que hay un problema de sobre temperatura,
    ...
    \item \texttt{bq76\_read\_cov\_fault\_reg()}: Lee registro de falla de sobre
        voltaje.
    \item \texttt{bq76\_read\_cuv\_fault\_reg()}: Lee registro de falla de bajo
        voltaje.
...
requerida para que el modelo pueda evolucionar de forma correcta. Esto fue 
realizado estableciendo una terminal del \acrshort{MCU} en alto, mientras se 
realiza una lectura del dispositivo, esto fue medido por un osciloscopio 
obteniendo un tiempo total de 968uS para medir las 6 celdas, el mismo ensayo se 
puede observar en la Figura \ref{bq76_v_cell_time}.
...
\subsection{Drivers \texttt{ina226}}

El driver \texttt{ina226} provee una \acrshort{API} que implementa las 
estructuras de datos, definiciones y funciones necesarias para la operación del 
\acrshort{CI} \emph{INA226} de la firma \emph{Texas Instruments} según los 
lineamientos descriptos en su correspondiente hoja de datos. 
...
implementa el driver para la operación.

\begin{itemize}
    \item \texttt{ina226\_init()}: Inicializa una instancia del driver ina226 y
        el \acrshort{CI} con los parámetros de configuración seleccionados.
    \item \texttt{ina226\_reset()}: Reinicia el sensor de corriente. Equivalente al
        power\textendash on reset.
    \item \texttt{ina226\_get\_current()}: Lee el valor de corriente medido,
        esta es una funci\'on que bloquea el hilo principal.
    \item \texttt{ina226\_get\_current\_dma()}: Lee el valor de corriente medido
        utilizando la interfaz \acrshort{DMA}, esta funci\'on no bloquea el hilo
        principal.
    \item \texttt{ina226\_get\_vbus()}: Lee el valor de tensión del bus.
    \item \texttt{ina226\_get\_vbus\_dma()}: Lee el valor de tensión del bus
        utilizando la interfaz \acrshort{DMA}.
    \item \texttt{ina226\_get\_vshunt()}: Lee el valor de tensión en la
        resistencia shunt de medición.
    \item \texttt{ina226\_get\_vshunt\_dma()}: Lee el valor de tensión en la
        resistencia shunt de medición utilizando la interfaz \acrshort{DMA}.
    \item \texttt{ina226\_get\_pwr()}: Lee registro de cálculo de potencia.
    \item \texttt{ina226\_get\_pwr\_dma()}: Lee registro de cálculo de potencia
        utilizando la interfaz \acrshort{DMA}.
    \item \texttt{ina226\_set\_avg()}: Configura el numero de mediciones del
        promedio de las mediciones.
    \item \texttt{ina226\_set\_vbus\_ct()}: Configura el tiempo de conversión de
        la tensión de bus.
    \item \texttt{ina226\_set\_vshunt\_ct()}: Configura el tiempo de conversión 
        de tensión shunt.
    \item \texttt{ina226\_set\_mode()}: Configura el modo de operación del
        dispositivo.
    \item \texttt{ina226\_set\_calibration()}: Calibra el sensor de corriente
        INA226. Configurando  el valor de resistencia shunt y de corriente 
        máxima.
    \item \texttt{ina226\_set\_mask\_enable()}: Configura los registros de
...
escritura de los pines correspondientes a cada instancia, según se describe en 
la implementación ejemplo que presenta el driver. La inicialización y puesta en
...
\begin{itemize}
    \item \texttt{bq24\_init()}: Inicializa una instancia del driver bq24 con
        los handlers de los pines del GPIO seleccionados por le usuario.
    \item \texttt{bq24\_read\_charger\_status()}: Lee el estado de operación
        actual del Cargador de Batería.
    \item \texttt{bq24\_is\_power\_good()}: Lee el pin de estado de alimentación
        del Cargador de Batería.
    \item \texttt{bq24\_is\_charger\_enabled()}: Lee el pin de estado de
        habilitación del cargador de batería.
    \item \texttt{bq24\_enable\_charge()}: Habilita el proceso de carga.
    \item \texttt{bq24\_disable\_charge()}: Deshabilita el proceso de carga.
\end{itemize}
...
\section{Implementaci\'on del filtro de Kalman}\label{dev_fw_kalman}
...
diseñada provee dos funciones al desarrollador:
...
\begin{itemize}
    \item \texttt{kalman\_filter\_init}: Esta funci\'on toma como par\'ametros un
        ...
        del sistema indicando si el filtro fue inicializado de forma correcta.
    ...
    \item \texttt{kalman\_filter\_step}: Esta funci\'on es la encargada de
        ...
        de esta librer\'ia tiene que acceder a la variable de inter\'es para sus
        objetivos.
\end{itemize}

Internamente, \'esta \acrshort{API} realiza cada operaci\'on que implementa el 
filtro, descriptas en la Secci\'on \ref{KalmanFilterMethod}, en las siguientes 
...
\section{\acrshort{API} del modelo de una celda}

\subsection{Desarrollo de la \acrshort{API}}
...
\begin{itemize}
    \item \texttt{calculate\_cell\_soc}: Dada la medici\'on actual del voltaje de
        ...
        paso y paso est\'a dado por como fue discretizado el modelo.
    \item \texttt{init\_cell\_model}: Esta funci\'on inicializa la estructura de
        ...
        voltaje de la celda, y calcula el \acrshort{SOC} inicial en base a este 
        valor.
    ...
    \item \texttt{battery\_model\_set\_r\_data}: Actualiza la matriz de
        incerteza de medici\'on del filtro de Kalman. Esta funci\'on debe ser
...
\subsection{Extensi\'on al pack de bater\'ias}

El modelo de la celda es extendido al pack bater\'ias descripta en la Secci\'on
\ref{Section_battery_pack}, donde se utiliza 6 modelos (\texttt{Cell}) y la 
corriente utilizada es dividida por 3, para reflejar el hecho de que el pack de 
bater\'ias tiene, por m\'odulo, 3 celdas conectadas en paralelo, esto es posible 
de realizar, ya que se considera que el modelo se encuentra linealizado en 
...
f\'acilmente para extender la cantidad de m\'odulos conectados en serie y en
paralelo, modificando las definiciones \texttt{SERIES\_CELLS} y
\texttt{PARALLEL\_CELLS}. A su vez la librer\'ia tiene dos funciones p\'ublicas:

\begin{itemize}
    \item \texttt{init\_battery\_pack}: Inicializa la cantidad de
        \texttt{SERIES\_CELLS} (en este caso, 6) de estructuras tipo 
        \texttt{Cell} tomando como referencia el \acrshort{OCV} de cada celda.
    \item \texttt{calc\_battery\_pack\_soc}: Dado el voltaje de cada celda y la
        corriente que circula a trav\'es del pack, estima el \acrshort{SOC} de
        cada celda.
\end{itemize}
...
no son las adecuadas se produciría el fen\'omeno de \emph{aliasing}.
...
valores de los polos conociendo los Autovalores de la matriz de evolución A.
...
Tomando el valor del máximo valor absoluto entre los elementos $A_{nn}$ de 
todas las matrices A, obtenidas para cada valor de \acrshort{SOC} se obtiene 
la Ecuación \ref{eq_polo_rapido}, que corresponde al valor del polo más rapido:
...
según la Ecuación \ref{ang_vs_frec}, se obtiene $F$ y, reemplazando en
\ref{nyquist_condition}, se obtiene la frecuencia mínima de muestreo $f_{smin}$ 
...
Los tiempos de procesamiento del filtro de Kalman y la obtenci\'on de datos
desde los sensores en conjunto de 10ms por lo que la frecuencia de muestreo de 
nuestro sistema es $100 Hz$ cumpliendo con la condición \ref{nyquist_condition}.

\chapter{Ensayos}\label{ensayos}
\thispagestyle{fancy}

En este pen\'ultimo Cap\'itulo se detallan los ensayos realizados en el 
\acrshort{BMS} utilizando tests unitarios, es decir, tests sobre el
\acrshort{BMS} que corren sobre datos simulados, que buscan determinar el tiempo
de ejecuci\'on de los distintos algoritmos como tambi\'en evaluar si los
resultados se condicen con las simulaciones de Matlab, como tambi\'en tests en 
campo, que implica la conexi\'on del pack de bater\'ias en conjunto a una carga 
activa para simular la descarga de corriente.

\section{Ensayos unitarios}
...
estado bajo. El resultado se puede observar en la Figura \ref{fig:kalman_time}
donde se puede apreciar el pulso que expone la terminal del \acrshort{MCU} es de
305uS de duraci\'on, es decir 50,83uS por celda, un n\'umero aceptable en 
comparaci\'on a los resultados que se presentan en \cite{kalman_filter_embedded} 
que utilizan el mismo hardware pero con un modelo con menos variables obteniendo
un resultado de 1.2mS en realizar un c\'alculo del filtro.
...
simulaciones que se desarrollaron en la Secci\'on \ref{dev_batt_model}. La idea
de estos ensayos con las simulaciones del filtro es contrastar si la
portabilidad del algoritmo desde MatLAB al microcontrolador funciona de forma
correcta.

\section{Ensayo del disposito}
...
la implementación de hardware del cargador, se optó por hacer un balanceo
est\'atico sin el uso de un cargador externo.
...
no debería ocurrir en estas celdas.
...
\begin{figure}[h!]
	\begin{center}
		...
		\caption{Ensayo de equalización de celdas desbalanceadas sin filtro de 
        Kalman.}
		\label{Test_BQ76_1}
	\end{center}
\end{figure}
...
alternativa del filtro mostrando como el mismo maximiza las estad\'isticas de 
probabilidad.
...
Ecuaci\'on \ref{error_estimacion}:
...
m\'axima probabilidad dada por la Ecuaci\'on \ref{probabilidad_y_x} se encuentra 
expresada en la Ecuaci\'on \ref{max_likelihood};
...
\'ultima variable se asume como un ruido Gaussiano con una covarianza conocida y 
...
se obtiene la Ecuaci\'on \ref{est_update_eq}.
...
\ref{e_mse_def_exp_kalman_indep}, se obtiene la Ecuaci\'on \ref{prior_error_cov};
...
transpuesta, podemos re-escribir la Ecuaci\'on \ref{cov_error_expanded_prior} en 
la Ecuaci\'on \ref{cov_error_expanded_prior_trpsed};
...
definida en la Ecuaci\'on \ref{cov_pred_medicion};
