...
escalabilidad, puediendo extenderse la capacidad del \acrshor{BMS} conectanndo
...
de las diferentes opciones se pueden observar en la Tabla
\ref{tabla_comparativa_ics}. 
...
192 celdas. El mismo integra un frontend anal\'ogico (\acrshort{AFE}, del
...
bater\'ias de 6 celdas en serie se utiliz\'o un solo \acrshort{CI} sin necesidad
de acoplar otro para extender su capacidad. En la Figura
\ref{fig:bq76_implementation_sch} se puede apreciar la implementaci\'on del
dispositivo, donde se observa el puerto SPI norte conectado a la tensi\'on total
del pack de bater\'ias ($\mathrm{V_{BATT}}$) y el puerto sur a masa
($\mathrm{GND}$), anulando ambos perif\'ericos. Por otra parte, en la Figura
\ref{fig:bq76_implementation_uncouple} se detallan los capacitores de
desacople especificados por el fabricante \cite{BQ76PL536A} necesarios para el
...
El circuito de potenci del balanceo consta de un transistor N-FET
...
El algoritmo de balanceo apropiado es implementado y procesado
en la unidad de procesamiento del \acrshort{BMS}.  

La corriente de balanceo $I_{bal}$ es fija y está definida por la resistencia de
carga $R_{Load} \equiv R21$, dependiendo de la tensión en bornes de la celda a
balancear. 

\begin{align}
    I_{bal (max)} &= \frac{V_{OCV(max)}}{R_{load}+R_{DS (on)}}\\
    &= \frac{4.2}{(47 + 1.7)\ohm}\\
    &\approx 86mA
\end{align}

\begin{align}
    I_{bal (min)} &= \frac{V_{OCV(min)}}{R_{load}+R_{DS (on)}}\\
    &= \frac{2.5}{(47 + 1.7)\ohm}\\
    &\approx 51mA
\end{align}
...
Cada una de las entradas de ADC de medición de tensión de las celdas requieren la
preciencia de un filtro anti-aliasing. El propósito del filtro antialiasing es
eliminar toda presencia, antes de hacer el muestreo, de las frecuencias
superiores a frecuencia muestreo e indeseados. Consiste en una resistencia de $1
K\ohm$ en serie con un capacitor de $100 nF$ en los pines VC6 - VC2 y de $1 \mu
F$ en VC1 y VC0.

La sección de potencia del balanceador cuenta con otro circuito de proteccion de
sus integrados. Circuito dumper para evitar transitorios de sobre voltaje por la
