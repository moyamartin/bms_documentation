...
Por estas caracteristicas los tiempos de apagado y encendido o tiempos de conmutación no son nulos y se tienen que tener en cuenta en aplicaciones de alta frecuencia, si bien nuestro interruptor no es un dispositivo que trabaje en alta frecuencia, nos propusimos minimmizar el tiempo de apagado para que la desconexión de la batería y la carga sea prácticamente instantaneo y lleve al sistema a un estado de seguridad rapidamente evitando así posibles daños en el equipo.
...
La capacidad de \emph{gate-source} $C_{GS}$ tiene el mayor valor, y es prácticamente constante ya que queda determinada por la geometría del \emph{gate} y la metalización del \emph{source}.
...
Se asume que $C_{GD}$ tiene valor $C_{GD1}$ para tensiones de $V_{DS}$ mayores que la tensión de comando de \emph{gate} (llave todavía abierta) y un valor $C_{GD2}$ mucho mayor para tensiones menores que $V_{GS}$ de comando (llave cerrándose) (Mohan et al. 1995).
	...
	\begin{center}
		\begin{minipage}[c]{0.95\textwidth}
			...
			\begin{circuitikz}[american]
				...
				\draw (0,5) 	to [short,-*]				(0,6) node[above] {$V_{bat}$};
...
Nuestra carga es un motor por lo que no podemos ignorar su caracter inductivo a la hora de realizar el análisis siguiente:
