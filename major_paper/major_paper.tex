...
Para el diseño del interruptor se tienen en cuenta las características del sistema las cuales son:\\
...
Si bien los dispositivos semiconductores de potencia generalmente son modelados como interruptores ideales, los mismos poseen características no ideales, como son resitencias intrínsecas, capacidades parásitas, etc.\\

Por estas caracteristicas los tiempos de apagado y encendido o tiempos de conmutación no son nulos y se tienen que tener en cuenta en aplicaciones de alta frecuencia, si bien nuestro interruptor no es un dispositivo que trabaje en alta frecuencia, nos propusimos minimizar el tiempo de apagado para que la desconexión de la batería y la carga sea prácticamente instantaneo y lleve al sistema a un estado de seguridad rapidamente evitando así posibles daños en el equipo.\\
...
Para analizar el comportamiento durante la conmutación debemos tener en cuenta los modelos en los estados por los cuales transiciona el transistor, los mismos se muestran en la figura   \ref{modelo_mosfet}.
...
Por otro lado, respecto a las capacidades intrínsecas del transistor sabemos que la capacidad de \emph{gate-source} $C_{GS}$ tiene el mayor valor, y es prácticamente constante ya que queda determinada por la geometría del \emph{gate} y la metalización del \emph{source}.
...
Se asume que $C_{GD}$ tiene valor $C_{GD1}$ para tensiones de $V_{DS}$ mayores que la tensión de comando de \emph{gate}, $V_{gg1}$, (llave todavía abierta) y un valor $C_{GD2}$ mucho mayor para tensiones menores que $V_{GS}$ de comando (llave cerrándose) (Mohan et al. 1995).\\
...
Teniendo en cuenta el punto anterior y que nuestra carga es un motor por lo que no podemos ignorar su caracter inductivo a la hora de realizar el análisis siguiente:\\
...
A partir de aquí $V_{DS}$ comienza a aumentar, mientras $C_{gd}$ se carga linealmente con corriente $I_g=V_{gsa}/R_g$ en principio con pendiente baja $I_g/C_{gd2}$ y para $V_{ds} > V_{gg1}$, siendo $V_{gg1}$ el valor umbra mencionado en \ref{aprox_CGD}, con pendiente más alta $I_g/C_{gd1}$.
...
El IRF9310PbF adquiere el menor valor de $R_{DS(on)}$ cuando $V_{gs} = 10V$ según la hoja de datos, mientras que la tensión que soporta $V_{gs-max}=20V$ por lo que en conducción se calculan $R_{64}$ y $R_{65}$ para $V_{gs}=15V$.
