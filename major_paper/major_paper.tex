...
\subsubsubsection{Desarrollo de Módelo Máquina de Estado Finitos}
Para llevar a cabo las tareas previamente mencionadas, se opto por un módelo de Máquina de estados debido a la capacidad de representar sistemas arbitrariamente complejos en un conjunto de estados y transiciones lo cual simplifica a posteriori la implementación, reduciendo la consigna a un grupo de tareas pequeñas y condiciones simples.

Con bases en la implementación de una máquina de estados en C \cite{Lafreniere2019}, se realizó una adaptación más sencilla para no correr código preparado para trabajar con sistemas complejos innecesario en este projecto.
...
\acrshort{MCU} y la librería \texttt{stm32f4xx\_hal\_spi} para acceder, a través
del periférico del microcontrolador, al canal de comunicación serie.

El \acrshort{CI} implementa una interfaz serie de alta velocidad \emph{SPI} para
la comunicación con la unidad de procesamiento.  Una descripción más acabada del
mismo puede encontrarse en la sección \ref{seq:bq26_selection}. El driver se
implementa bajo una filosofía de instanciado que permite atender y operar en
múltiples instancias correspondientes a múltiples balanceadores presentes en el
hardware bajo el único requisito de que estén comunicándose todos sobre el mismo
canal de comunicación \emph{SPI}. La inicialización y puesta en marcha del
periférico \emph{SPI} del \acrshort{MCU} es responsabilidad del usuario del
driver.
...
\begin{itemize}
    ...
    \item \texttt{bq76\_reset()}: Enviar comando reset a un dispositivo en
        particular.
    \item \texttt{bq76\_set\_address()}: Setear la dirección de un dispositivo.
    \item \texttt{bq76\_set\_adc\_control()}: Setear registro de control del ADC
        del  \acrshort{CI}
    \item \texttt{bq76\_set\_cb\_time()}: Setear tiempo maximo de balanceo de una
        celda.
    \item \texttt{bq76\_set\_funtion\_config()}: Seterar registro de configuración
        de funcionamiento.
    \item \texttt{bq76\_set\_io\_config()}: Setear registro de configuración i/o.
    \item \texttt{bq76\_set\_conv\_config()}: Setear voltaje de sobre tensión.
    \item \texttt{bq76\_set\_cuv\_config()}: Setear voltaje de tensión mínima.
    \item \texttt{bq76\_set\_covt\_config()}: Setear delay de alarma de voltaje de
        sobre tensión.
    \item \texttt{bq76\_set\_cuvt\_config()}: Seterar delay de alarma de tensión
        mínima.
    \item \texttt{bq76\_set\_ot\_config()}: Setear valor se sobre temperatura.
    \item \texttt{bq76\_set\_ott\_config()} Setear delay de alarma de sobre
        temperatura.
    \item \texttt{bq76\_read\_v\_cells()}: Lectura del voltaje de n celdas
    \item \texttt{bq76\_read\_v\_cells\_dma()}: Lectura del voltaje de n celdas
        mediante dma.
    \item \texttt{bq76\_swrqst\_adc\_convert()}: Lanzar inicio converción ADC en
        un dispositivo particular.
    \item \texttt{bq76\_bdcst\_adc\_convert()}: Lanzar inicio de converción ACD
        en todos los dispositivos instanciados con una dirección válida de comunicación.
    \item \texttt{bq76\_read\_alert\_reg()}: Leer registro de alertas.
    \item \texttt{bq76\_read\_fault\_reg()}: Leer registro de fallas.
    \item \texttt{bq76\_clear\_fault\_reg()}: Limpiar registro de fallas.
    \item \texttt{bq76\_clear\_alert\_reg()}: Limpiar registro de alertas.
    ...
    \item \texttt{bq76\_read\_cuv\_fault\_reg()}: Leer registro de falla de bajo
        voltaje.
    \item \texttt{bq76\_set\_balancing\_output()}: Balancear una salida n
        particular.
    \item \texttt{handle\_bq76\_dma\_callback()}: Handler interrupción DMA del
        driver bq76.
\end{itemize}

\subsubsubsection{Driver ina226}
...
\subsubsection{Implementaci\'on del filtro de Kalman}
